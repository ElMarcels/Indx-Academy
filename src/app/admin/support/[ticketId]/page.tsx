...
import {
  FiArrowLeft, FiSend, FiLoader, FiUser, FiShield, FiCheckCircle, FiLock, FiTag, FiAlertCircle,
} from 'react-icons/fi';
  ...
  return (
    <div className="py-12">
      <div className="section max-w-3xl mx-auto">
        <Link href="/admin/support" className="text-dark-400 hover:text-white text-sm mb-6 flex items-center gap-1.5 transition-colors">
          <FiArrowLeft size={14} /> Tickets de soporte
        </Link>
        ...
        <div className="card p-6 mb-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span className="text-xs text-dark-400">
                  de <span className="text-white">{ticket.user.name || ticket.user.email}</span>
                </span>
            ...
            >
              {closing ? <FiLoader size={12} className="animate-spin" /> : isOpen ? <FiCheckCircle size={12} /> : <FiLock size={12} />}
                    ...
                    <div className={`max-w-[75%] ${isOwn ? 'items-end' : ''}`}>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-medium text-dark-300">
                          {isStaff ? 'Staff (Tú)' : msg.sender.name || msg.sender.email}
                        </span>
          ...
          <div className="border-t border-dark-800/50 p-4">
            {isOpen ? (
              ...
              <div className="flex items-center justify-center gap-2 py-3 text-dark-500 text-sm">
                <FiLock size={14} />
                Ticket cerrado · <button onClick={toggleClose} className="text-brand-400 hover:text-brand-300 transition-colors">Reabrir</button>
              </div>
            )}
          </div>
        </div>
